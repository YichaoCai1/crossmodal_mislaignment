...
def get_data(dataset, selected_semantics, f1, f2, loss_func, dataloader_kwargs):
    loader = DataLoader(dataset, **dataloader_kwargs)
    iterator = InfiniteIterator(loader)
    labels_image = {v: [] for v in Multimodal3DIdent.FACTORS["image"].values()}
    labels_text = {v: [] for v in selected_semantics}
...
def main():
    ...
    # define dataloaders
    dataset = Multimodal3DIdent(args.datapath, bias_type=args.bias_type, bias_id=args.bias_id, mode="train", **dataset_kwargs)
    selected_semantics = dataset.selected_semantics
    ...
    # Generate indices (fixed order, no shuffling)
    ...
    # define text encoder
    sequence_length = dataset.max_sequence_length
    ...
    val_dict = get_data(val_dataset, selected_semantics, encoder_img, encoder_txt, loss_func, dataloader_kwargs)
    test_dict = get_data(test_dataset, selected_semantics, encoder_img, encoder_txt, loss_func, dataloader_kwargs)
    ...
    for m in ["image", "text"]:
        ...
        discrete_factors_m = Multimodal3DIdent.DISCRETE_FACTORS[m].values()
        for ix, factor_name in factors_m.items():
            
            if m=="text" and factor_name not in selected_semantics:
                continue
            ...
            r2_linreg, r2_krreg, mcc_logreg, mcc_mlp = [np.nan] * 4

            # check if factor ix is discrete for modality m
            if factor_name in discrete_factors_m:
            ...
            if factor_type == "continuous":
                ...
                r2_krreg = evaluate_prediction(
                    MLPRegressor(max_iter=10000), r2_score, *data)
            ...
            results.append([ix, m, factor_name, factor_type,
                            r2_linreg, r2_krreg, mcc_logreg, mcc_mlp])
